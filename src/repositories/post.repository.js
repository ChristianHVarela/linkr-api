...
export const getPostsOrderByCreatedAtDesc = () => {
    return db.query('SELECT p.*, u.name AS user_name, u.image AS image_profile, pm.title AS title_metadata, pm.description AS description_metadata, pm.image_url AS image_metadata FROM posts p JOIN users u ON u.id = p.user_id JOIN posts_metadata pm ON pm.post_id = p.id ORDER BY p.created_at DESC LIMIT 20')
